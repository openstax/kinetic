  ...
  Api::V1::Bindings::ParticipantStudy.class_exec do
    ...
    def self.attributes_from_study_model(model, user)
      model.attributes_for_binding(self).tap do |attributes|
        ...
        attributes[:completed_at] = model.first_launched_study&.completed_at
        attributes[:aborted_at] = model.first_launched_study&.aborted_at
        attributes[:consented] = model.first_launched_study&.consent_granted
