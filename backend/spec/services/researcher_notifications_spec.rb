...
RSpec.describe ResearcherNotifications, type: :mailer do
  ...
  before do
    allow(UserInfo).to receive(:for_uuids).and_return([added_researcher_info_response, removed_researcher_info_response])
  end
  ...
  it 'notifies researchers that were added and removed from study' do
    ...
    expect(email1.subject).to match "You've been invited to a study"
