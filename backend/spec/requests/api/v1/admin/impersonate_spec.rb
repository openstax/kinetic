  ...
  describe 'POST' do
    context 'when admin is logged in' do
      ...
      it 'can impersonate a researcher' do
        ...
        expect(response).to redirect_to(Rails.application.credentials.frontend_url)
      end

      it 'can stop impersonating' do
        api_get 'admin/impersonate/stop'
        expect(session).to match(hash_excluding(:impersonating))
        expect(response).to redirect_to("#{Rails.application.credentials.frontend_url}/admin/impersonate")
      end
    end
  end
end
